...
public final class LlmClientFactory {
    public LlmClient create(AppConfig config) {
        ...
        return switch (config.provider()) {
            case OPENAI -> new OpenAiClient(config, httpClient, objectMapper, eventReader, errorMapper);
            case ANTHROPIC -> new AnthropicClient(config, httpClient, objectMapper, eventReader, errorMapper);
            case DEEPSEEK -> new DeepSeekClient(config, httpClient, objectMapper, eventReader, errorMapper);
        };
    }
}
